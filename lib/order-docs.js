/* ============================================================
   ...
   Donde viven:
     ...
     - Un renglon en la lista OrderDocuments (la creo el dueño):
       OrderID, ClientID, ItemId (id del archivo en SharePoint),
       UploadedBy ('Client' | 'Office'), UploaderName, SizeBytes. Asi se
       sabe quien subio que (el cliente solo borra lo suyo).
       OJO (26/09/2026): OrderID es la columna Title renombrada por el
       dueño, asi que su nombre INTERNO es 'Title' (Graph solo acepta ese).
       No hay columna para el nombre del archivo: el nombre que se ve sale
       del archivo en SharePoint ("<stamp> <nombre>", sin el stamp).
...
/* OrderID = columna Title renombrada (nombre interno 'Title'). */
const orderIdOf = row => String(((row && row.fields) || {}).Title || '').trim();

/* "2026-09-26_142501-ab12 Contrato.pdf" -> "Contrato.pdf" */
const shownName = n => String(n || '').replace(/^\d{4}-\d{2}-\d{2}_\d{6}-[a-z0-9]+ /, '') || 'Document';

function docOf(it, name) {
  const f = it.fields || {};
  return {
    id: String(it.id),
    orderId: orderIdOf(it),
    clientId: f.ClientID || '',
    name: name || 'Document',
...
/* Nombres de los archivos, 20 por llamada ($batch de Graph).
   -> { itemId: nombre que se ve }. Si falla, esos salen como 'Document'. */
async function fileNames(graph, itemIds) {
  const out = {};
  if (!itemIds.length) return out;
  let driveId;
  try { driveId = await graph.getDriveId(); } catch (e) { console.error('order-docs: drive failed', e.message); return out; }
  const chunks = [];
  for (let i = 0; i < itemIds.length; i += 20) chunks.push(itemIds.slice(i, i + 20));
  await Promise.all(chunks.map(async ids => {
    try {
      const r = await graph.graphFetch('/$batch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ requests: ids.map((id, i) => ({ id: String(i), method: 'GET', url: '/drives/' + driveId + '/items/' + encodeURIComponent(id) + '?$select=id,name' })) })
      });
      ((r && r.responses) || []).forEach(x => { if (x.status === 200 && x.body) out[ids[Number(x.id)]] = shownName(x.body.name); });
    } catch (e) { console.error('order-docs: names failed', e.message); }
  }));
  return out;
}
...
async function listDocs(graph, opts) {
  ...
  if (opts.orderIds) { const set = new Set(opts.orderIds); rows = rows.filter(it => set.has(orderIdOf(it))); }
  const names = await fileNames(graph, rows.map(it => (it.fields || {}).ItemId).filter(Boolean));
  const docs = rows.map(it => docOf(it, names[(it.fields || {}).ItemId]));
...
async function finishUpload(graph, order, itemId, fileName, by, uploaderName) {
  ...
  let created;
  try {
    created = await graph.createListItem(DOCS_LIST, {
      Title: order.OrderID,
    ...
    /* 26/09/2026: sin renglon el archivo no sale en ningun portal, asi que
       se borra para que no queden archivos sueltos en la carpeta. */
    await graph.graphFetch('/drives/' + driveId + '/items/' + encodeURIComponent(itemId), { method: 'DELETE' }, true).catch(() => {});
    const cols = /not recognized/i.test(e.message) ? await columnProblem(graph) : '';
    console.error('order-docs: list row failed', e.message, cols);
    throw Object.assign(new Error(cols ? 'The OrderDocuments list in SharePoint does not have these columns: ' + cols + '.' : e.message), { status: 500, code: 'DOC_LIST' });
  }
  return docOf({ id: created.id, fields: Object.assign({ Created: new Date().toISOString() }, created.fields || {}, {
    Title: order.OrderID, ClientID: order.ClientID || '',
    UploadedBy: by === 'Client' ? 'Client' : 'Office', UploaderName: uploaderName || '', SizeBytes: item.size || 0
  }) }, shownName(item.name));
}

/* 26/09/2026: el dueño creo las columnas y seguia saliendo "Field 'OrderID'
   is not recognized". Graph solo acepta el nombre INTERNO exacto (se fija
   al crear la columna; renombrarla no lo cambia). Esto lee las columnas
   reales de la lista y dice cual falta y, si hay una parecida, como se
   llama en SharePoint (ej. 'OrderId', 'Order_x0020_ID', 'OrderID0'). */
const DOC_COLUMNS = ['ClientID', 'ItemId', 'UploadedBy', 'UploaderName', 'SizeBytes'];
const normCol = n => String(n || '').toLowerCase().replace(/_x0020_/g, '').replace(/[\s_]/g, '').replace(/\d+$/, '');

async function columnProblem(graph) {
  try {
    const data = await graph.graphFetch(graph.siteListPath(DOCS_LIST).replace(/\/items$/, '/columns'));
    const cols = (data && data.value) || [];
    const names = new Set(cols.map(c => c.name));
    return DOC_COLUMNS.filter(n => !names.has(n)).map(n => {
      const near = cols.find(c => normCol(c.name) === normCol(n) || normCol(c.displayName) === normCol(n));
      return near ? n + " (SharePoint has it as '" + near.name + "')" : n + ' (missing)';
    }).join(', ');
  } catch (e) {
    console.error('order-docs: columns check failed', e.message);
    return '';
  }
}
...
module.exports = { DOCS_LIST, MAX_BYTES, docsFolder, listDocs, groupDocs, findOrder, getRow, startUpload, finishUpload, viewUrls, deleteDoc, docOf, orderIdOf };
