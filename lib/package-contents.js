...
/* Settings guarda los JSON largos en trozos de 250 caracteres:
   Key = clave (trozo 1), clave#2, clave#3, ... (Admin los escribe asi
   porque la columna Value no acepta textos largos). Se pegan en orden. */
function joinSetting(rows, key) {
  const re = new RegExp('^' + key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(?:#(\\d+))?$');
  return rows.filter(it => it.fields && re.test(String(it.fields.Key || '')))
    .map(it => ({ v: it.fields.Value || '', n: Number((String(it.fields.Key).match(re) || [])[1] || 1) }))
    .sort((a, b) => a.n - b.n).map(x => x.v).join('');
}
async function readJsonSettings(keys) {
  ...
  keys.forEach(k => {
    let v = {};
    const text = joinSetting(rows, k);
    if (text) { try { v = JSON.parse(text) || {}; } catch (e) { v = {}; } }
    out[k] = v;
  });
  return out;
}
