...
/* Lista real donde Admin (Admingsocd.com) guarda las asignaciones de
   Scheduling -- OrderAssignments/TechID nunca se llena en el flujo
   normal, es la de verdad. */
...
/* "Assign by service" (Admin > Scheduling/Active) -- una fila por
   ...
   creada en SharePoint que usan Admingsocd.com y ordersgsocd.com,
   mismo nombre. */
...
/* Suscripciones de notificaciones push -- una por dispositivo/navegador
   por tecnico (un mismo tecnico puede tener varias, si usa el celular
   y la computadora). Lista compartida con Admingsocd.com (mismo sitio
   de SharePoint), que es quien realmente manda las notificaciones. */
const PUSH_SUBSCRIPTIONS_LIST = 'PushSubscriptions';

/* Acceso por QR sin login -- confirmado con el usuario: la oficina
   escanea el QR CON el celular del empleado, ahi mismo, en persona.
   SetupToken se usa UNA sola vez (para reclamar el dispositivo);
   DeviceToken es el permanente, el que vive guardado en localStorage
   del celular del empleado para siempre (hasta que se revoque). Un
   solo DeviceToken activo por tecnico -- generar uno nuevo apaga el
   anterior solo. */
const TECH_DEVICE_TOKENS_LIST = 'TechDeviceTokens';

/* Registro de CADA foto/video subido (aparte del archivo mismo en
   SharePoint) -- quien lo subio, cuando, y donde, para el reporte de
   Times and Routes (pendiente) sin tener que leer metadata de
   archivos. */
...
module.exports = {
  ...
  SERVICES_CATALOG_LIST, TECHS_LIST, ORDER_ASSIGNMENTS_LIST, SCHEDULING_LIST, SERVICE_ASSIGNMENTS_LIST, ORDERS_FOLDER,
  RECURRING_SERVICES_LIST, RECURRING_ASSIGNMENTS_LIST, RECURRING_LOG_LIST, PUSH_SUBSCRIPTIONS_LIST,
  TECH_DEVICE_TOKENS_LIST, TECH_PHOTO_LOG_LIST,
